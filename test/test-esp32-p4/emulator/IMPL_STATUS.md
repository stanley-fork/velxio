...
✅ **Phase 2.J.uart** — **Investigación: app RUNNING, no stuck**. Análisis exhaustivo del trace (15K lines / 20s wall):
  - **NO uart_tx calls** en trace — ningún acceso a 0x4FC00054 trampoline ni 0x500CA000 UART base.
  - **Disasm de `__esp_system_init_fn_init_show_app_info`** revela que NO es el banner — sólo computa `app_elf_sha256_str` (no printf). El banner real (`Build:...`/`ESP-IDF X.Y.Z`) está en `bootloader_print_banner` que sólo corre en bootloader, no en app.
  - **App está en `soc_get_available_memory_regions`** iterando con `memcpy` (9 calls en 20s wall) procesando ~30 memory regions. NO es loop infinito — progresando lento por TCG overhead.
  - Hot fns (call counts): `pmu_hp_system_init` × 108, `esp_cpu_configure_region_protection` × 106, `pmu_init` × 95, `rtc_clk_cal_internal` × 76.
  - **Final gate identificado**: para llegar a `app_main` y ver UART output, la app necesita FreeRTOS scheduler running, que requiere **timer interrupts**. Phase 2.D actual sólo tiene CLIC backing-RAM sin IRQ delivery al CPU. Phase 2.K es la siguiente fase mayor (~200-400 LOC) que conecta interrupts al CPU + SYSTIMER tick + UART RX IRQ.

**Estado del milestone**: App ELF runtime CORRE 60+ IDF init functions correctamente. La arquitectura del emulador (CPU + memory + peripheral stubs + cache MMU) está validada end-to-end. El "Hello world" / Arduino blink visible está a una fase de distancia (Phase 2.K).
✅ **Phase 2.K.systimer** `e408a196d1` — **SYSTIMER snapshot protocol fix**. El IDF `systimer_hal_get_counter_value` escribe TIMER_UPDATE bit 31 a OP_REG, polleó TIMER_VALUE_VALID bit 30 hasta set, lee VAL_HI/LO. Nuestro stub original devolvía 0 en OP_REG → infinite poll. Fix: OP_REG read siempre devuelve bit 30 set; OP_REG write con bit 31 captura snapshot; VAL_HI/LO lee snapshot. Soporte unit 0 + unit 1.
✅ **Phase 2.K.init_skip** `53f8358abb` — **Skip do_system_init_fn → app llega a C++ static init + pthread + FreeRTOS port**. NOP los 2 `jal do_system_init_fn` en `start_cpu0` (offsets 0x400091F2 y 0x4000923A). Skipea ~30 init functions slow bajo TCG. App ahora alcanza el C++ init array — ejecuta static constructors usando `pthread_mutex_init/lock/unlock`, `xPortSetInterruptMaskFromISR`, `vPortClearInterruptMaskFromISR`, `xPortEnterCriticalTimeout`. Trade-off: heap_caps no completo, peripheral clocks no enabled — pero CPU+memory+cache MMU+SYSTIMER+flash funcional. Próximo blocker: `vTaskStartScheduler` necesita CLIC IRQ delivery + timer tick (Phase 2.K.scheduler, ~150-300 LOC).
