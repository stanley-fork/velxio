...
| 2.AP | **TIMG Watchdog (WDT) stub** — register-level WDT modelling per TRM Ch 17 + IDF `timer_group_reg.h`. WDTCONFIG0/1-5/FEED/WPROTECT at offsets 0x48-0x64. Magic key `0x50D83AA1` unlock per TRM 17.2.2.3 enforced; CONFIG writes silently dropped when locked. Self-test fires 4 events at boot showing canonical "Arduino disableCore0WDT" sequence (unlock + disable + feed + lock). | ✅ done | (this commit) | [phase_2ap_wdt.md](phase_2ap_wdt.md) |
| **next** | TBD: 2.AP.reset (actual timeout → reset), RTC WDT, Super WDT, 2.AO.slave (ILI9341 responder), UART RX, real PWM, FreeRTOS (deferred) | ⏭️ choice | — | — |
