...
| 2.AD | **ADC peripheral** at `0x500DE000` returns 4-second sawtooth via `QEMU_CLOCK_REALTIME`. Demo blob reads ADC each loop and forwards to LEDC duty — physically realistic "analog input drives LED brightness" pipeline. JSON stream now carries 4 event types: start, pin, ledc, adc. | ✅ done | (this commit) | [phase_2ad_adc.md](phase_2ad_adc.md) |
