...
| 2.AR | **HW Random Number Generator (RNG)** at `0x500FC400` — TRM Ch 33. Reads return random uint32 from QEMU host PRNG. JSON `event:"rng"` per access (50ms throttled, with running count). Foundation for `esp_random()` / `random()` / TLS / secure boot. Self-test reads 3 values at boot. | ✅ done | (this commit) | [phase_2ar_rng.md](phase_2ar_rng.md) |
| **next** | TBD: 2.AS (peripheral address relocation to real-silicon), 2.AP.reset, RTC WDT, Super WDT, 2.AO.slave, UART RX, real PWM, FreeRTOS (deferred) | ⏭️ choice | — | — |
