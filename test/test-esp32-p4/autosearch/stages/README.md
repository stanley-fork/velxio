...
| 2.AQ | **TIMG1 WDT self-test mirror** — single-line addition: machine init now calls `esp32p4_timg_wdt_self_test(&ms->timg1)` after the TIMG0 one. JSON stream shows 8 wdt events (4 per group) instead of 4. Both groups demonstrate Arduino's `disableCoreNWDT()` pattern. | ✅ done | (this commit) | [phase_2aq_timg1_wdt.md](phase_2aq_timg1_wdt.md) |
| **next** | TBD: 2.AP.reset (actual timeout → reset), RTC WDT, Super WDT, 2.AO.slave, UART RX, real PWM, RNG, FreeRTOS (deferred) | ⏭️ choice | — | — |
